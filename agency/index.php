...
include '../config/Database.php';
    ...
    if(!isset( $_SESSION['isAgencyLoggedIn'])){
        header("Location: login.php?error=Login to add a car");
                exit();
    }
?>
...
<div class="row">
    <div class="container">
        <a href="addcar.php" class="btn btn-secondary btn-lg">+ List Car</a>
    </div>
           ...
           while($row = mysqli_fetch_assoc($result)){
            ?>
                <div class="card col-md-4 mx-3 px-3">
                    <h5 class="card-header"><?php echo $row['model_name'] ?></h5>
                    <div class="card-body">
                        <h5 class="card-title"><?php echo $row['number'] ?></h5>
                        <p class="card-text">With supporting text below as a natural lead-in to additional content.</p>
                        <a href="#" class="btn btn-primary"><?php echo $row['name'] ?></a>
                    </div>
                    </div>
            <?php
           }
